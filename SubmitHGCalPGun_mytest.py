...
#eosExec = 'eos'
eosExec = 'xrdfs root://cmseos.fnal.gov '
...
def getInputFileList(DASquery,inPath, inSubDir, local, pattern):
    print(DASquery,inPath,inSubDir, local, pattern)
    inputList = []
    if not DASquery:
        if (local):
...
#            inputList = [f for f in processCmd(eosExec + ' ls ' + inPath+'/'+inSubDir+'/').split('\n')   if ( (processCmd(eosExec + ' fileinfo ' + inPath+'/'+inSubDir+'/' + f).split(':')[0].lstrip() == 'File') and (fnmatch.fnmatch(f, pattern)))]
            inputList = [f.split('/')[-1] for f in processCmd(eosExec + ' ls ' + inPath+'/'+inSubDir+'/').split('\n')   if (  (fnmatch.fnmatch(f, pattern)))]
...
def submitHGCalProduction(*args, **kwargs):
  ...
  # prepare dir for GSD outputs locally or at EOS
    if (opt.LOCAL):
        ...
        recoInputPrefix = 'root://cmseos.fnal.gov/'+opt.eosArea+'/'+opt.inDir+'/'+previousDataTier+'/'
    ...
    if (opt.DTIER == 'RECO' or opt.DTIER == 'NTUP'):
        ...
        #commonFileNamePrefix = inputFilesList[0].split("/")[-1].split("_", 1)[0]
        print("commonFileNamePrefix",commonFileNamePrefix)
    ...
    for job in range(1,int(njobs)+1):
        ...
        write_condorjob.write('use_x509userproxy       = true \n')    ###added for proxy
        write_condorjob.write('executable  = '+currentDir+'/SubmitFileGSD_mytest.sh \n')
